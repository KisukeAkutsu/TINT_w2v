...
def object_TINT_edge_correspondence_heatmap(target,source):
    ...
    df_edge_corr = pd.read_csv(Corr_DIR+"Date_all_seed_6000_{}_{}_full_anti_1_iter_1000_correspondence.csv".format(target,source),header=0,encoding="utf-8")
...
def tri_TINT_edge_correspondence_heatmap(target,source,tri_dom,tri_cod):
    ...
    df_edge_corr = pd.read_csv(Corr_DIR+"FOREDGE_Date_all_seed_6000_{}_{}_{}_{}_forced_anti_1_iter_1000_correspondence.csv".format(target,source,tri_dom,tri_cod),header=0,encoding="utf-8")
...
def load_object_correspondence_data(fname,target,source):
    ...
    A_node_data = make_node_data(target, node_data)
    B_node_data = make_node_data(source, node_data)
    matrix = make_count_matrix(A_node_data, B_node_data, df_edge_corr)
...
def human_tri_data_correlation_to_csv(target,source,seed,r_function=pearsonr):
    ...
    T_images = make_node_data("蝶", node_data)
    S_images = make_node_data("踊り子", node_data)
    T_images.append("NA")
